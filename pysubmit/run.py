...
hdfs="hdfs://"
ip="20.0.9.20"
small_ratings_file = hdfs+ip+":54310/datasets/ml-latest-small/ratings.csv"
...
#small_movies_file = "hdfs://20.0.9.10:54310/datasets/ml-latest-small/movies.csv"
small_movies_file = hdfs+ip+":54310/datasets/ml-latest-small/movies.csv"
...
#Split ratings data random:
# 60% training (20% validation 20% testing) => parsing to eliminate ratings
...
for rank in ranks:
    #Training with training_RDD
    model = ALS.train(training_RDD, rank, seed=seed, iterations=iterations,
                      lambda_=regularization_parameter)
    #Predictions with validation_RDD previously created
    predictions = model.predictAll(validation_for_predict_RDD).map(lambda r: ((r[0], r[1]), r[2]))
    #Join between Rates and Predictions to find error
...
#Now that we know whats is our best_rank with less error we re-train to have a model
#And Calculate RMSE(root-mean-square error)
